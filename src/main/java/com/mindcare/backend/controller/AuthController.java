...
public ResponseEntity<?> loginUser(@RequestBody LoginRequest loginRequest) {
    ...
    try {
        ...
        if (authenticatedUser != null) {
            ...
            return ResponseEntity.ok(authenticatedUser); // Return user info instead of token
        } else {
        ...
        return ResponseEntity.status(500).body("Internal Server Error: " + e.getMessage()); // Log detailed error
    }
}
